...
export const getUserCadres = async (req, res) => {
    try {
        ...
        if (!cin || !role) {
            return res.status(400).json({ message: "Informations utilisateur manquantes (cin, role requis)" });
        }

        let cadres;
        switch (role) {
            case 'etudiant':
                // Solution alternative pour contourner le problème de jointure
                ...
                cadres = etudiant ? etudiant.participants : [];
                break;
            ...
            case 'professionnel':
                // Solution alternative pour professionnel
                cadres = await Cadre.findAll({
                    include: [{
                        model: Professionnel,
                        as: 'professionnel',
